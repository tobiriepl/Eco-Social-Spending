...
ggsave(filename = "Analysis/Plots/1a_need_spending_wid.png", plot = last_plot(),  width = 10, height = 12, units = "in", bg = "transparent", dpi = 300)
...
data_reg_gov <- tibble(
  sector = c("Health & Environment", "Social Protection", "Education & Recreation", 
             "Housing", "Industry", "Order & Defense", "Public Administration"), 
  coefficient =  c(7.56, 3.25, 5.22, 15.60, 3.24, -8.35, 26.81)   
  # total spending cap data.   health_envi:  7.56***, soc: 3.25***,  edu_recr: 5.22***, hous: 15.60***, indu: 3.24***, ord_def: -8.35***, admin: 26.81***     
) |>                                                                             
  mutate(sector = factor(sector, levels = rev(c(  "Education & Recreation",  
                                                  "Social Protection", 
                                                  "Public Administration",    
...
data_reg_hh <- tibble(
  sector = c("Health & Environment", "Social Protection", "Education & Recreation", 
             "Housing", "Industry", "Order & Defense", "Public Administration"), 
  coefficient =  c(6.06, -5.25, 1.35, 3.44, -4.40, -19.10, 6.04)   # gdp_cap          
  #  health_envi: 6.06***, soc: -5.25, edu_recr: 1.36, hous: 3.44***, indu: -4.40***, ord_def: -19.10**, admin: 6.04***
) |>                                                                             
  mutate(sector = factor(sector, levels = rev(c(  "Education & Recreation",  
                                                  "Social Protection", 
                                                  "Public Administration",    
...
\nby Tobias Riepl, Richard Bärnthaler, Lena Kilian, Diana Ivanova
